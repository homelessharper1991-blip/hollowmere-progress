...
- **2026-07-09 (later)** — **FOUR OWNER RULINGS:** ① PERMADEATH YES, with resurrection contracts — telepathic cleric-call (SRD *Sending*), player clerics get first claim, NPC casters are fallback, RAW time windows + gold set the real bounds. ② OPEN PVP YES, governed by full crime/bounty simulation. ③ Observed/unobserved = outcome-equivalent batch resolution (delegated; DM-practice-accurate). ④ Prior kd-tree notes SCRAPPED; owner intent = kd-tree for dynamic server-cell assignment, settled in a dedicated review. **NEW DIRECTIVE: SERVER CELLS** (multi-database world, atlas module, portal-aligned transfers) — promoted to V2, staged in `features/cells/` (DESIGN.md #9).
...
- [V2][XL] **SERVER CELLS (owner directive — promoted from V3)** — the world runs on multiple server cells, never one assumed process: cell = zone set on its own SpacetimeDB database; atlas/directory module (zone→cell→URI, character locator); idempotent character-transfer protocol at zone portals; bridge-side transparent handoff; boundaries portal-aligned only (no cross-cell combat — no cross-database transactions exist). **Dynamic zone→cell reassignment by load; the partitioning algorithm (owner intends kd-tree) is settled in a dedicated owner design review — prior kd notes scrapped per owner.** Dev story: all cells on one local standalone instance; production spreads across nodes. Staged now in `features/cells/`. Single-module ceiling measurement (bot harness) still runs first — it tells us when cells must activate, not whether to build them.
