...
- [V3][XL] **Sharding & the no-cap truth** — measure single-module ceiling with bots; logical instances; module-per-region with character transfer third (a ferry ride's UX). §1.11's arithmetic raises the ceiling dramatically — measure, don't assume. **Region-boundary selection (owner note, 2026-07-09): when drawing module-per-region boundaries, use kd-style adaptive partitioning over population/activity density** so each module carries roughly equal simulation load — an offline rebalancing computation, not a runtime structure. (This is the owner's kd-tree suggestion landing where it genuinely fits; runtime proximity queries stay on grid cells per §1.11(b).)
