...
- **2026-07-12 (late)** - **READS RIGHT:** sprite scale tuned to SRD size categories (rats finally look like rats next to a dwarf, the Warden stands a head taller), name labels fade with distance so crowds stop shouting, character sheet/journal/inventory verified pixel-by-pixel (and their numbers no longer say "Level 1.0"). Screenshot QA now also opens every panel each run. Repo self-sufficiency audit done ahead of the July 13 handover: QA toolkit, export pipeline, supervisor and the public-page clone all live in permanent paths.

- **2026-07-12 (night)** - **THE FRONT DOOR:** intro cinematic live - Ken Burns pans over the six keyart panels with the authored narration cards and the Warden title reveal, letterboxed, any-key skip (automation bypasses it). Login is now a proper dark-souls menu: flooded-ward keyart, slow fade from black, amber title. Earlier today the first eyes-on screenshot pass caught and fixed five invisible-to-logs bugs (WebGL texture cap blanking all terrain, zero-size UI roots pinning the whole HUD to a corner, a silent action-bar crash, ghost characters, spawn camping). Screenshot QA is now part of every wake: headless Chrome plays the game and the frames get LOOKED at.

- **2026-07-12 (evening)** - **COMBAT FEEL + DENSER WORLD:** attack swings now play synced to server combat events (attacker faces the victim, one-shot animation), damaged entities flash red, and the camera kicks when a hit lands on you - all QA-verified via first-fire breadcrumbs. World density up ~50% (5 rats, 3 ghouls, 6 bandits, 5 cultists + fanatic) via a new no-wipe SyncSpawns maintenance reducer (content updates land without touching player data). README rewritten with the real 3-command quickstart and corrected Flare CC-BY-SA art attribution.

- **2026-07-12 (later)** - **THE GAME HAS A FACE:** full HUD shipped - player frame (HP/AC/conditions), target frame, action bar with per-class spell buttons (attack + spells + potion, keybinds 1-5, slot counts, greyed without slots), zone chat panel, NWN-style combat log, journal (J) / character sheet (C) / inventory (I) panels, death overlay with respawn countdown, and the dialogue panel - the authored campaign dialogue is now actually visible to players (it previously existed only in the data layer). Verified by autopilot: wizard build kills by fire bolt from range, drinks a potion, walks to Reeve Maera and opens her intro dialogue (4 options rendered). Test suites hardened to retry through legitimate SRD defeats - the dice are real and rat gangs sometimes win.
